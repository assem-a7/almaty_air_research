"""
PM2.5 1-Hour-Ahead Forecasting Model
=====================================

This module implements a production-quality machine learning pipeline for
forecasting PM2.5 concentrations 1 hour ahead in Almaty, Kazakhstan.
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("modeling_forecast_pm25_t1")
...
def load_and_validate_data(logger: logging.Logger) -> pd.DataFrame:
    ...
    logger.info(f"  PM2.5 mean: {df['pm25'].mean():.2f} µg/m³")
    
    # Validate pm10 if exists
...
def create_features(df: pd.DataFrame, logger: logging.Logger) -> pd.DataFrame:
    """
    Create time-series features for PM2.5 forecasting.
    
    Features include:
    - Calendar features (hour, day_of_week, month, year)
    - Lag features for pm25 (1, 2, 3, 6, 12, 24, 48, 168 hours)
    - Rolling statistics (mean, std) for 24h and 168h windows
    - PM10 lag features if available
    - Target: pm25 shifted -1 hour (1-hour-ahead prediction)
    ...
    # PM2.5 lag features
    logger.info("\n2. Creating PM2.5 lag features...")
    lag_hours = [1, 2, 3, 6, 12, 24, 48, 168]
    
    for lag in lag_hours:
        df[f'pm25_lag_{lag}'] = df['pm25'].shift(lag)
        logger.info(f"  ✓ Created: pm25_lag_{lag}")
    
    # Rolling statistics (using shift to prevent leakage)
    logger.info("\n3. Creating PM2.5 rolling statistics...")
    
    # 24-hour rolling stats
    df['pm25_roll_mean_24'] = df['pm25'].shift(1).rolling(window=24, min_periods=12).mean()
    df['pm25_roll_std_24'] = df['pm25'].shift(1).rolling(window=24, min_periods=12).std()
    df['pm25_roll_min_24'] = df['pm25'].shift(1).rolling(window=24, min_periods=12).min()
    df['pm25_roll_max_24'] = df['pm25'].shift(1).rolling(window=24, min_periods=12).max()
    logger.info("  ✓ Created: pm25_roll_mean_24, pm25_roll_std_24, pm25_roll_min_24, pm25_roll_max_24")
    
    # 168-hour (7-day) rolling stats
    df['pm25_roll_mean_168'] = df['pm25'].shift(1).rolling(window=168, min_periods=84).mean()
    df['pm25_roll_std_168'] = df['pm25'].shift(1).rolling(window=168, min_periods=84).std()
    logger.info("  ✓ Created: pm25_roll_mean_168, pm25_roll_std_168")
    
    # Rate of change features
    df['pm25_diff_1'] = df['pm25'].diff(1)
    df['pm25_diff_24'] = df['pm25'].diff(24)
    logger.info("  ✓ Created: pm25_diff_1, pm25_diff_24 (rate of change)")
    
    # PM10 lag features (if available)
    if 'pm10' in df.columns and df['pm10'].notna().sum() > 0:
        logger.info("\n4. Creating PM10 lag features...")
        ...
        logger.info("  ✓ Created: pm10_lag_1, pm10_lag_24, pm10_roll_mean_24")
    
    # Target variable: 1-hour-ahead PM2.5
    logger.info("\n5. Creating target variable...")
    df['target_pm25_t_plus_1'] = df['pm25'].shift(-1)
    logger.info("  ✓ Created: target_pm25_t_plus_1 (pm25 shifted -1 hour)")
    
    # Drop rows with NaN target (last row)
    initial_len = len(df)
    df = df.dropna(subset=['target_pm25_t_plus_1'])
...
def prepare_features_and_target(
    ...
    exclude_cols = ['datetime', 'target_pm25_t_plus_1', 'pm25', 'date']
    if 'pm10' in train_df.columns:
        exclude_cols.append('pm10')
    ...
    y_train = train_df['target_pm25_t_plus_1'].values
    y_test = test_df['target_pm25_t_plus_1'].values
...
def evaluate_naive_baseline(
    ...
    The baseline predicts next hour PM2.5 as current hour PM2.5.
    ...
    logger.info("Description: Current PM2.5 value predicts next hour")
    
    # Baseline prediction: current pm25 value
    y_pred_naive = test_df_filtered['pm25'].values
...
def time_series_cv_evaluation(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(X), 1):
        ...
        if isinstance(X, pd.DataFrame):
            X_train_fold = X.iloc[train_idx].to_numpy()
            X_val_fold   = X.iloc[val_idx].to_numpy()
        else:
...
def train_and_evaluate_models(
    ...
    (
        ...
        False  # scaling already inside pipeline
    ),
    (
        'Ridge Regression',
        Pipeline([
...
# ============================================================================
# 9. VISUALIZATION (plots remain the same, just updated signatures)
# ============================================================================
...
def plot_predictions_vs_actual(y_test, results, test_dates, output_path, logger):
    """Plot predicted vs actual (code unchanged)."""
    best_model_name = min(results, key=lambda k: results[k]['test_rmse'])
    ...
    ax2.set_xlabel('Actual PM2.5 (µg/m³)', fontweight='bold')
    ax2.set_ylabel('Predicted PM2.5 (µg/m³)', fontweight='bold')
...
def plot_residual_analysis(y_test, results, output_path, logger):
    """Plot residual analysis (code unchanged)."""
    best_model_name = min(results, key=lambda k: results[k]['test_rmse'])
    y_pred = results[best_model_name]['y_test_pred']
...
def plot_feature_importance(results, feature_names, output_path, logger):
    """Plot feature importance (code unchanged)."""
...
def main():
    ...
    logger = setup_logger(LOGS_DIR / "modeling_forecast_pm25_t1.log")
    
    logger.info("=" * 80)
    logger.info("PM2.5 1-HOUR-AHEAD FORECASTING PIPELINE")
    logger.info("=" * 80)
    logger.info(f"Location: {CITY}, {COUNTRY}")
    logger.info(f"Timezone: {TIMEZONE}")
    logger.info(f"Target: PM2.5 concentration 1 hour ahead")
    ...
    try:
        ...
        plot_model_comparison(results, baseline_metrics, FIGURES_DIR / "model_comparison_pm25_t1.png", logger)
        plot_predictions_vs_actual(y_test, results, test_dates, FIGURES_DIR / "predictions_vs_actual_pm25_t1.png", logger)
        plot_residual_analysis(y_test, results, FIGURES_DIR / "residual_analysis_pm25_t1.png", logger)
        plot_feature_importance(results, feature_names, FIGURES_DIR / "feature_importance_pm25_t1.png", logger)
        
        logger.info("\n" + "=" * 80)
        logger.info("SAVING RESULTS")
        logger.info("=" * 80)
        save_results_table(results, baseline_metrics, TABLES_DIR / "model_performance_pm25_t1.csv", logger)
