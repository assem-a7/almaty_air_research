# Almaty Air Quality Forecasting

Machine Learning project focused on analyzing and forecasting PM2.5 and PM10 air pollution levels in Almaty, Kazakhstan.

This repository implements a structured data science pipeline including data acquisition, exploratory analysis, feature engineering, and time series forecasting.

---

## Project Overview

Location: Almaty, Kazakhstan  
Target variables: PM2.5, PM10  
Objective: Short-term pollution forecasting and temporal pattern analysis  

Air pollution is a major environmental and public health issue in Almaty.  
This project models pollution dynamics and builds predictive models to analyze and forecast air quality levels.

---

## Methodology

The project follows a research-oriented workflow:

1. Data Collection  
   - Automated data download  
   - Config-driven parameters  
   - Structured storage  

2. Exploratory Data Analysis (EDA)  
   - Distribution analysis  
   - Correlation analysis  
   - Seasonal trends  
   - Weekly and diurnal patterns  

3. Feature Engineering  
   - Lag features  
   - Rolling statistics  
   - Time-based features  

4. Modeling  
   - PM2.5 forecasting model  
   - PM10 forecasting model  
   - Model comparison  
   - Residual diagnostics  

---

## Key Results

### Time Series Overview
![Time Series](reports/figures/time_series_full.png)

### Seasonal Patterns
![Seasonal Patterns](reports/figures/seasonal_patterns.png)

### Model Comparison (PM2.5)
![Model Comparison](reports/figures/model_comparison_pm25_t1.png)

### Predictions vs Actual (PM2.5)
![Predictions](reports/figures/predictions_vs_actual_pm25_t1.png)

### Feature Importance
![Feature Importance](reports/figures/feature_importance_pm25_t1.png)

---

## Project Structure

almaty_air_research/
│
├── reports/
│   └── figures/                  (final visual outputs)
│
├── src/
│   ├── config.py
│   ├── data_download.py
│   ├── eda.py
│   ├── exploratory_analysis.py
│   ├── modeling_forecast_pm10_t1.py
│   ├── modeling_forecast_pm25_t1.py
│   ├── utils.py
│   └── tests_test_data_download.py
│
├── src/notebooks/                (research notebooks)
├── requirements.txt
├── setup.py
└── README.md

---

## How to Run

1. Install dependencies:

pip install -r requirements.txt

2. Run forecasting scripts:

python src/modeling_forecast_pm25_t1.py

or

python src/modeling_forecast_pm10_t1.py

3. Optional: Run notebooks

Open Jupyter Notebook and execute notebooks inside src/notebooks/.

---

## Technologies Used

- Python  
- pandas  
- NumPy  
- matplotlib  
- scikit-learn  
- statsmodels  
- Jupyter Notebook  

---

## Notes

- Raw datasets are not included in this repository.  
- All visual results are reproducible using the provided pipeline.  
- Configuration parameters are centralized in src/config.py.  

---

## Author

Abdikarim Assem  
Eurasian National University (ENU)  
Astana, Kazakhstan